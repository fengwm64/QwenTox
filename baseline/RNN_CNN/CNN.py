...
# 构建CNN模型
def build_cnn_model(max_features, embed_size=128, max_len=384):
    model = Sequential([
...
# 构建多尺度CNN模型
def build_multi_scale_cnn_model(max_features, embed_size=128, max_len=384):
    # 输入层
    inputs = Input(shape=(max_len,))

    # 嵌入层
    embedding = Embedding(max_features, embed_size, input_length=max_len)(inputs)
    ...
    for size in filter_sizes:
        conv = Conv1D(num_filters, size, activation='relu', padding='same')(embedding)
        conv = BatchNormalization()(conv)
        conv = Conv1D(num_filters, size, activation='relu', padding='same')(conv)
        conv = BatchNormalization()(conv)
    ...
    # 全连接层
    x = Dense(128, activation='relu', kernel_initializer='he_normal')(concatenated)
...
def save_metrics_to_json(val_metrics, test_metrics, label_stats, multilabel_stats, history):
    results = {
        ...
        'model_info': {
            ...
            'dropout_rates': [0.3, 0.3, 0.4, 0.3, 0.2],
            'optimizer': 'Adam',
            'learning_rate': 0.001
        },
        'data_info': {
...
def main():
    ...
    # 构建模型 - 可以选择使用简单CNN或多尺度CNN
    print("\n构建CNN模型...")

    model = build_multi_scale_cnn_model(
        max_features=20000,
        embed_size=128,
        max_len=256
    )
    ...
    # 保存结果
    submission.to_csv(os.path.join(tables_dir, 'cnn_submission.csv'), index=False)
    print("提交文件已保存!")

    # 保存模型
    trained_model.save(os.path.join(models_dir, 'toxic_comment_cnn_model'))
    print("模型已保存!")
